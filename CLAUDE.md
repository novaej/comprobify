...
**Legal documents and tenant acceptance (ADR-018):** `legal_documents` (migrations 057, 059) stores the Markdown source for `TERMS`/`PRIVACY`/`DPA` — never PDF. HTML is rendered on demand via `markdown-it` (`legal-document.service.js`'s `renderHtml()`/`getCurrentHtml()`), so the only persisted artifact is text plus a SHA-256 `content_hash`; nothing to keep in sync if the renderer changes. Public `GET /v1/legal/documents` lists the current version of each type; `GET /v1/legal/documents/:type` serves the rendered HTML (`text/html`). `POST /v1/admin/legal-documents` (admin-only, JSON `{ documentType, version, contentMarkdown }`) publishes a new version — "current" is always the newest row per type by `created_at`, no separate flag to keep in sync. `renderHtml()` also accepts a `values` object for `{{token}}` placeholder substitution (unmatched tokens are left visible, not blanked) — built for future per-tenant personalization (e.g. a DPA copy with the Client's actual name baked in) but not yet wired into any endpoint.

`POST /v1/register` requires `termsVersion`, validated against the currently published `TERMS` version before any expensive work runs (`legalAcceptanceService.validateTermsVersion()`, called before P12 parsing) — skipped only if nothing has been published yet (pre-launch, nothing authoritative to check against). On success, one `legal_acceptances` row (migration 060) is written per currently-published document type — TERMS, PRIVACY, and DPA all logged independently in the same call (`legalAcceptanceService.recordAcceptance()`), sharing one `accepted_at`/`ip`/`user_agent` since a single signup checkbox accepts the whole bundle. `legal_acceptances` is append-only and is the actual audit trail (full history survives every re-acceptance); `tenants.legal_accepted_at`/`legal_version` are kept only as a denormalized "latest at a glance" cache mirroring the TERMS row. Admin-created tenants (`POST /v1/admin/tenants`) skip `termsVersion` entirely — not self-service signup, no acceptance to gate.

`GET /v1/tenants/legal-status` (authenticated) compares the tenant's latest acceptance **per document type** against what's currently published and returns exactly which type(s) drifted (`{ needsAcceptance, outdated: [{ documentType, currentVersion, acceptedVersion, url }] }`) — this is what catches a DPA-only republish without requiring ToS/Privacy to also bump a shared version string. `POST /v1/tenants/accept-legal` (`{ termsVersion }`) re-validates against current `TERMS` and records a fresh acceptance row for every currently-published type, capturing `req.ip`/the `user-agent` header automatically (no extra fields needed from the frontend).

The DPA has no separate acceptance UI — its own text states it's accepted by virtue of accepting the ToS (incorporation by reference, see `docs/legal/terminos-de-servicio.md` §1), and `recordAcceptance()` logs a DPA row alongside TERMS/PRIVACY in the same call regardless of whether the Client ever reads it. `docs/legal/*.md` holds the drafted Spanish text for all three documents — still placeholder-laden (`[Tu nombre completo]`, `[RUC]`, etc.) pending real entity details and an actual lawyer review pass before anything gets published via the admin endpoint.
...
| `src/models/legal-acceptance.model.js` / `src/services/legal-acceptance.service.js` | `legal_acceptances` audit trail — `create`, `findLatestByTenantAndType`, `findAllByTenant`; `validateTermsVersion` (fail-fast check used by both registration and re-acceptance), `recordAcceptance` (one row per currently-published type per event), `getStatus` (per-type drift detection backing `GET /v1/tenants/legal-status`) |
