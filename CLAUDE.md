...
**Legal documents and tenant acceptance (ADR-018):** Two-layer design — a template store (`legal_documents`, migrations 057/059) and per-tenant personalized snapshots (`tenant_legal_documents`, migration 061).

`legal_documents` stores the Markdown source for `TERMS`/`PRIVACY`/`DPA`. Content comes from `docs/legal/*.md` read at publish time (`POST /v1/admin/legal-documents`, JSON `{ documentType, version }`, no file upload) — the draft header blockquote is stripped by `stripDraftHeader()` before storage. Public `GET /v1/legal/documents` lists current versions; `GET /v1/legal/documents/:type` serves rendered HTML via `markdown-it` with a disclaimer notice prepended (not stored in content). Supports `{{token}}` placeholder substitution (`{{fechaVersion}}`, `{{fechaDocumento}}`, `{{cliente.razonSocial}}`, `{{cliente.ruc}}`).

`tenant_legal_documents` stores one **personalized, immutable** Markdown snapshot per `(tenant_id, document_type, template_version)` — all `{{}}` tokens are resolved at generation time (DPA includes the Client's actual `business_name` and `ruc`). Status: `PENDING` (generated, not yet accepted) or `ACCEPTED`. Generation happens: (a) at registration as a fire-and-forget background call once the issuer row exists; (b) lazily when `GET /v1/tenants/legal-acceptance` or `GET /v1/tenants/legal-documents/:type` is called and the tenant has no row for the current template version — so publishing a new template surfaces automatically without any admin job. `UNIQUE (tenant_id, document_type, template_version)` with `ON CONFLICT DO NOTHING` makes all generation calls idempotent.

`POST /v1/register` requires `termsVersion`, validated against the current `TERMS` template version before P12 parsing (`tenantLegalDocumentService.validateTermsVersion()`) — skipped if nothing is published yet. `POST /v1/tenants/legal-acceptance` (`{ termsVersion }`) accepts all PENDING instances at once, recording `accepted_at`/`ip`/`user_agent` on each row. Old ACCEPTED rows survive every re-acceptance — full history is preserved. `tenants.legal_accepted_at`/`legal_version` are a denormalized cache only.

`GET /v1/tenants/legal-acceptance` returns per-type drift detection (`{ needsAcceptance, outdated: [{ documentType, currentVersion, acceptedVersion, status, url }] }`) — third-party integrators should poll this periodically to know when their tenant needs to re-accept updated documents. `GET /v1/tenants/legal-documents` lists all instances (history); `GET /v1/tenants/legal-documents/:type` renders the tenant's personalized HTML (the stored snapshot — no substitution at read time, already baked in) with the disclaimer prepended.

`POST /v1/tenants/promote` is **gated** on all three document types being `ACCEPTED` (`hasAllAccepted()` → `403 LEGAL_ACCEPTANCE_REQUIRED` if not). Admin override: `POST /v1/admin/tenants/:id/legal-documents` generates instances for pre-existing tenants. Admin-created tenants skip `termsVersion` entirely. `docs/legal/*.md` holds the source templates (Spanish, entity info filled in, `{{cliente.*}}` tokens awaiting per-tenant substitution).

**User-facing promotion:** `POST /v1/tenants/promote` (authenticated) — checks tenant is ACTIVE, checks all legal documents are `ACCEPTED` (`403 LEGAL_ACCEPTANCE_REQUIRED` if not), flips `tenants.sandbox = false`, seeds production sequentials for all issuers × document types, revokes all sandbox API keys, and creates matching production keys (one per revoked sandbox key, same label). Returns `{ apiKeys: [{ label, apiKey }] }` — all tokens shown once, store immediately. This is one-way. Admin override: `POST /v1/admin/tenants/:id/promote` (skips both ACTIVE status and legal acceptance checks).
...
26. Computing a subscription's next `current_period_start`/`current_period_end` from `new Date()` ("now") instead of the OLD `current_period_end` — an admin reviewing a renewal/upgrade proof a few days early or late would then drift the billing date forward or backward every cycle. Always anchor to the old `current_period_end` via the shared `addBillingPeriod()` helper in `subscription.service.js`, used by `activateIfLinked` (only place "now" is correct — there is no prior period), `applyRenewalIfLinked`, and `applyScheduledTierChanges`'s period rollover.
...
| `src/models/tenant.model.js` | Tenant CRUD — `create`, `findByEmail`, `findByVerificationToken`, `activate`, `promote`, `updateTier`, `updateStatus`, `updateVerificationToken`, `updateVerificationEmailSent`, `updateVerificationEmailStatus`, `updateLegalAcceptance` (denormalized `legal_accepted_at`/`legal_version` cache only — see `legal-acceptance.service.js` for the actual audit trail), `findByVerificationEmailMessageId`, `countBranchesByTenantId`, `countIssuePointsByBranch` |
...
| `src/services/tenant.service.js` | Tenant mutations — `updateLanguage`, `promote` (with ACTIVE status check), `getLegalStatus`/`acceptLegal` (delegate to `legal-acceptance.service.js`) |
| `src/services/subscription.service.js` | Subscription/payment pipeline — `createSubscription` (tier + billing interval, returns `bankTransfer` info), `createSubscriptionForTenant` (tenant-facing wrapper for `POST /v1/subscriptions` — gates on `tenant.status === ACTIVE`, then delegates to `createSubscription`; works regardless of sandbox status), `requestTierChange` (tenant-initiated upgrade/downgrade — see CLAUDE.md "Tier changes"), `submitPaymentProof` (tenant-facing, ownership-checked; fires the operator proof-submitted email), `getPaymentProof`, `reviewPayment` (one decision: VERIFIED/REJECTED; fires the tenant-facing notification + email), `linkInvoice` (routes a `TIER_CHANGE` or `RENEWAL` payment's invoice link separately from the subscription's own), `activateIfLinked` / `applyTierChangeIfLinked` / `applyRenewalIfLinked` (all three fire-and-forget from `document-transmission.service.js` on every authorization), `applyScheduledTierChanges` (admin job — applies due downgrades, rolls their period forward), `processDueRenewals` (admin job, **must run after** `applyScheduledTierChanges` — opens renewal reminders, expires unpaid subscriptions to FREE; see "Recurring renewals" above), `addBillingPeriod` (shared period-math helper, anchored to a fixed date, never "now"), `cancelSubscription` |
...
| `src/controllers/tenant.controller.js` | Thin handlers for `GET /v1/tenants/me`, `PATCH /v1/tenants/language`, `POST /v1/tenants/promote`, `GET /v1/tenants/legal-acceptance`, and `POST /v1/tenants/legal-acceptance` (captures `req.ip`/`user-agent`) |
...
| `src/models/legal-document.model.js` / `src/services/legal-document.service.js` | `legal_documents` CRUD (`create`, `findCurrentByType`, `findAllCurrent` — "current" = newest row per type) plus `renderHtml`/`getCurrentHtml` (Markdown → HTML via `markdown-it`, never persisted) and `substitutePlaceholders` (`{{token}}` replacement, unmatched tokens left visible) |
| `src/controllers/legal-document.controller.js` / `src/routes/legal-documents.routes.js` / `src/validators/legal-document.validator.js` | Public (no auth, no rate limiter) `GET /v1/legal/documents` (list current versions) and `GET /v1/legal/documents/:type` (rendered HTML) |
| `src/models/tenant-legal-document.model.js` / `src/services/tenant-legal-document.service.js` | Per-tenant personalized document snapshots — `create` (with `ON CONFLICT DO NOTHING`), `findLatestByTenantAndType`, `findAllByTenant`, `acceptAllPendingByTenant`; service: `generateForTenant` (resolves all `{{}}` tokens at generation time; auto-looks up issuer if not provided), `getStatus` (lazy-generates missing rows then returns per-type drift), `hasAllAccepted` (used by promotion gate), `validateTermsVersion`, `acceptAll`, `listForTenant`, `renderForTenant` |
