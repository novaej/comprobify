...
| `src/services/subscription.service.js` | Subscription/payment pipeline — `createSubscription` (tier + billing interval, returns `bankTransfer` info), `createSubscriptionForTenant` (tenant-facing wrapper for `POST /v1/subscriptions` — gates on `tenant.status === ACTIVE`, then delegates to `createSubscription`; works regardless of sandbox status), `requestTierChange` (tenant-initiated upgrade/downgrade — see CLAUDE.md "Tier changes"), `submitPaymentProof` (tenant-facing, ownership-checked), `getPaymentProof`, `reviewPayment` (one decision: VERIFIED/REJECTED), `linkInvoice` (routes a `TIER_CHANGE` payment's invoice link separately from the subscription's own), `activateIfLinked` / `applyTierChangeIfLinked` (both fire-and-forget from `document-transmission.service.js` on every authorization), `applyScheduledTierChanges` (admin job — applies due downgrades), `cancelSubscription` |
| `src/models/subscription.model.js` / `src/models/payment.model.js` | CRUD for `subscriptions`/`payments` — `db.query()` directly (not issuer-scoped, same precedent as `tenant-event.model.js`) |
