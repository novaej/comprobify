...
| `src/constants/subscription-tiers.js` | Tier definitions: `documentQuota`, `maxBranches`, `maxIssuePointsPerBranch`, rate limits, `allowedDocumentTypes`, `priceMonthlyUsd`/`overagePerDocumentUsd` (display/billing reference only — not yet enforced, no payment gateway exists) |
...
| `src/models/subscription.model.js` / `src/models/payment.model.js` | CRUD for `subscriptions`/`payments` — `db.query()` directly (not issuer-scoped, same precedent as `tenant-event.model.js`). `subscription.model.js` adds `findDueForRenewalReminder`/`findExpiredPastGrace`; `payment.model.js` adds `findPendingRenewalBySubscriptionId` (mirrors `findPendingTierChangeBySubscriptionId`) |
