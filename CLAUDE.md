...
**Subscription + payment pipeline (manual, admin-only):** `subscriptions` and `payments` tables track a tenant's paid-tier billing. **A subscription only becomes `ACTIVE` once its linked invoice document is SRI-`AUTHORIZED` — never merely on payment** (deliberate: granting paid access against an unauthorized invoice is exactly the kind of gap this product exists to close for everyone else). Flow: `POST /v1/admin/tenants/:id/subscriptions` creates a subscription (`PENDING_PAYMENT`) + a `payments` row priced from `TIERS[tier].priceMonthlyUsd` → `PATCH /v1/admin/payments/:id/report` / `/verify` / `/reject` track the manual SPI bank transfer (`verify` moves the subscription to `PAYMENT_RECEIVED`; doesn't require `report` to have run first) → `PATCH /v1/admin/subscriptions/:id/link-invoice` (`{ accessKey }`) attaches the self-billed invoice document (`invoice_document_id`, resolved via `documentModel.findByAccessKey()` with no issuer scoping — `documents.id` is never exposed in any response, so `accessKey` is the only identifier this could use), moving the subscription to `INVOICE_PROCESSING` → `document-transmission.service.js`'s `checkAuthorization()` fires `subscriptionService.activateIfLinked(documentId)` fire-and-forget on every authorization (same pattern as the existing notification/email side effects); it's a no-op for the vast majority of documents not linked to any subscription, and flips the matching subscription to `ACTIVE` + calls `tenantModel.updateTier()` to actually grant the tier/quota. There is no tenant-facing self-service yet (no frontend) — every endpoint is admin-only. The schema deliberately has **no payment-gateway-specific columns** — no Kushki (or any vendor) references exist until a gateway is actually decided and built; those will land in their own dedicated migration at that point, not speculatively now (see NEXT_STEPS.md #9, blocked on a registered legal entity existing).
...
| `src/services/subscription.service.js` | Manual subscription/payment pipeline — `createSubscription`, `reportPayment`/`verifyPayment`/`rejectPayment`, `linkInvoice`, `activateIfLinked` (called fire-and-forget from `document-transmission.service.js` on every authorization), `cancelSubscription` |
...
| `db/migrations/` | SQL migration files 001–052 (031: RLS; 033: sandbox schema; 034: api_key environment; 042: api_keys tenant-scoped; 043: sandbox moved from issuers to tenants; 044–047: notifications + webhooks; 048: term unit catalog; 049: drop issuer environment; 050: issuer logo; 051: generic `updated_at` trigger; 052: subscriptions + payments) |
