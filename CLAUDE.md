...
| `src/services/tenant.service.js` | Tenant mutations — `updateLanguage`, `promote` (with ACTIVE status check), `getAgreementStatus`/`acceptAgreements` (delegate to `agreements.service.js`) |
...
| `src/controllers/tenant.controller.js` | Thin handlers for `GET /v1/tenants/me`, `PATCH /v1/tenants/language`, `POST /v1/tenants/promote`, `GET /v1/tenants/agreements`, and `POST /v1/tenants/agreements` (captures `req.ip`/`user-agent`) |
| `src/routes/tenants.routes.js` | Authenticated tenant routes: get current tenant, language update, promotion, legal acceptance status + re-acceptance |
