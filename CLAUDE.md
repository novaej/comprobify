...
| `src/services/subscription.service.js` | Subscription/payment pipeline — `createSubscription` (tier + billing interval, returns `bankTransfer` info), `submitPaymentProof` (tenant-facing, ownership-checked), `getPaymentProof`, `reviewPayment` (one decision: VERIFIED/REJECTED), `linkInvoice`, `activateIfLinked` (fire-and-forget from `document-transmission.service.js` on every authorization), `cancelSubscription` |
...
| `src/controllers/subscription.controller.js` / `src/routes/subscriptions.routes.js` | Tenant-facing `GET /v1/subscriptions/me` — full subscription/payment history, `authenticate` middleware |
