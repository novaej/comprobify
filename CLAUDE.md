...
`tenant_legal_documents` stores one **personalized, immutable** Markdown snapshot per `(tenant_id, document_type, template_version)` — all `{{}}` tokens are resolved at generation time (DPA includes the Client's actual `business_name` and `ruc`). Status: `PENDING` (generated, not yet accepted) or `ACCEPTED`. Generation happens: (a) at registration as a fire-and-forget background call once the issuer row exists; (b) lazily when `GET /v1/tenants/legal-status` or `GET /v1/tenants/legal-documents/:type` is called and the tenant has no row for the current template version — so publishing a new template surfaces automatically without any admin job. `UNIQUE (tenant_id, document_type, template_version)` with `ON CONFLICT DO NOTHING` makes all generation calls idempotent.

`POST /v1/register` requires `termsVersion`, validated against the current `TERMS` template version before P12 parsing (`tenantLegalDocumentService.validateTermsVersion()`) — skipped if nothing is published yet. `POST /v1/tenants/accept-legal` (`{ termsVersion }`) accepts all PENDING instances at once, recording `accepted_at`/`ip`/`user_agent` on each row. Old ACCEPTED rows survive every re-acceptance — full history is preserved. `tenants.legal_accepted_at`/`legal_version` are a denormalized cache only.

`GET /v1/tenants/legal-status` returns per-type drift detection (`{ needsAcceptance, outdated: [{ documentType, currentVersion, acceptedVersion, status, url }] }`) — third-party integrators should poll this periodically to know when their tenant needs to re-accept updated documents. `GET /v1/tenants/legal-documents` lists all instances (history); `GET /v1/tenants/legal-documents/:type` renders the tenant's personalized HTML (the stored snapshot — no substitution at read time, already baked in) with the disclaimer prepended.
...
| `src/controllers/tenant.controller.js` | Thin handlers for `GET /v1/tenants/me`, `PATCH /v1/tenants/language`, `POST /v1/tenants/promote`, `GET /v1/tenants/legal-status`, and `POST /v1/tenants/accept-legal` (captures `req.ip`/`user-agent`) |
