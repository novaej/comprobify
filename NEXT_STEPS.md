...
**Status (2026-09-12): first in-house holistic pass complete — see `docs/security-audit-2026-09-12.md` for the full findings, organized by severity.** Three High-severity items came out of it: GitHub Environment protection rules are actually absent despite `docs/production-readiness-checklist.md` claiming otherwise (verified live via `gh api` — item 88 below turned out to be a real, unresolved gap, not just something to double-check), there's no real disaster-recovery/backup process for the production database, and the encryption-key-rotation script's shell-history gap is worse than originally scoped (it's specifically an incident-response tool, so it leaks the recovery key during the exact scenario — a suspected compromise — where that's most dangerous). The bullets below are the original pre-audit scoping list; treat `docs/security-audit-2026-09-12.md` as the authoritative current findings, not this list. This was an in-house pass, not a professional pentest — still worth deciding whether an external pentest is warranted before real tenant data is at stake.

Everything security-related so far has been reviewed piecemeal, as individual features shipped (RLS, rate limiting, attempt tracking, RFC 7807 error codes, etc. — see CLAUDE.md's various entries and ADR-012/ADR-026). Nobody had done one holistic pass looking for gaps *between* those individually-reviewed pieces. Two halves, both needed:
