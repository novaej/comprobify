...
## 13. Notification System Standardization — Per-Channel Subscription + Unconditional Ledger

**Priority: Medium — every new notification type currently reinvents delivery/idempotency ad hoc; this closes that gap before it compounds further**

Surfaced while building price-change notifications (see CLAUDE.md's "Price history + 30-day change notice"): the current pattern is `if (!notificationPreferenceModel.isEnabled(tenantId, type)) return null;` **before** creating the `notifications` row, then a completely separate, ungated code path in `email.service.js` decides whether to email. Two problems fall out of that:

1. **`notifications` can't be used as a "did this happen" record for most types**, because its own existence is conditional on a preference that has nothing to do with whether the underlying event occurred.
2. **Email and in-app are governed by two unrelated mechanisms** — one preference row gates in-app creation; email is either unconditional (payment/subscription emails) or doesn't exist as a concept for that type. There's no single place a tenant (or a frontend) can see "here's what I'm subscribed to, per channel."

**Already shipped as a scoped preview, for `PRICE_CHANGE_ANNOUNCED` only** (see CLAUDE.md's "Price history + 30-day change notice", ADR-023): `src/constants/non-subscribable-notification-types.js` is a plain array of "mandatory" types; `createPriceChangeAnnounced()` skips the preference gate entirely; `GET/PATCH /v1/notifications/preferences` never show or accept a preference row for a type in that list; and `tierPriceModel.findUnnotifiedPendingForTenant` checks `notifications` directly (`type='PRICE_CHANGE_ANNOUNCED' AND metadata->>'tierPriceId'`) as its idempotency source — no ledger table, since unconditional creation is what makes that safe. This item generalizes exactly that shape to the other 6 types, replacing the plain array with a richer catalog and making preference genuinely per-channel instead of a single on/off.

**What:**

1. **Enrich the existing `NotificationTypes` catalog in code, not a new DB table.** `src/constants/notification-types.js` becomes a richer map — `{ type, supportsEmail, supportsInApp, mandatory }` per type — mirroring how `subscription-tiers.js` already holds structural, code-defined metadata rather than a DB row per tier. No catalog table needed; this is capability metadata, not tenant data. `non-subscribable-notification-types.js`'s plain array becomes redundant once this lands (derive the mandatory list by filtering the catalog instead) and can be deleted.
2. **`mandatory: true` types are not subscribable at all** — sent to every tenant on every channel they support, no opt-out, no preference row ever created or accepted for them. `PRICE_CHANGE_ANNOUNCED` already works this way (see above); this step is about generalizing the mechanism (currently a hardcoded array check) into the richer catalog, and adding the "surface as present-but-locked" UX to `GET /v1/notifications/preferences` that the current minimal version (which omits the type from the list entirely) doesn't bother with.
3. **`notification_preferences` gains per-channel granularity** — `(tenant_id, type, channel, enabled)` with `channel IN ('EMAIL', 'IN_APP')` and a `UNIQUE(tenant_id, type, channel)` constraint, replacing today's single `enabled` per `(tenant_id, type)`. Same opt-out-by-default semantics as today (no row = enabled) for non-mandatory types; no rows are ever written for mandatory types. `notificationPreferenceModel.isEnabled(tenantId, type, channel)` gains the channel parameter; `upsertMany` takes `{ type, channel, enabled }[]` and rejects mandatory types at the validator layer.
4. **`notifications` row creation becomes unconditional for every type** — the `if (!enabled) return null` gate is removed from the remaining 6 creation call sites (`createDocumentAuthorized`, `createPaymentReviewed`, `createSubscriptionRenewalDue`, `createSubscriptionExpired`, the two `runCertChecksForTenant` alerts). `createPriceChangeAnnounced` already has no such gate. The row itself becomes the unconditional ledger for all 7 types — the answer to "did X happen for tenant Y" no longer depends on any subscription state.
5. **Channel preference moves to read/send time instead of create time:**
   - `GET /v1/notifications` filters to rows where the tenant currently has `IN_APP` enabled for that row's `type` (mandatory types always included) — visibility, not existence, is what preference controls now. A tenant re-subscribing doesn't retroactively fabricate history, but does start seeing new rows again immediately.
   - Email sending (the effect handler, e.g. today's `INVOICE_AUTHORIZED_EMAIL`/`PRICE_CHANGE_EMAIL`-style handlers) checks `EMAIL` channel preference before calling `emailService.sendX()`, except for `mandatory` types which always send (`PRICE_CHANGE_EMAIL` already always sends).
   - Webhook fan-out is unaffected — it already has its own per-endpoint subscription list (`webhookDeliveryModel`/`findSubscribedByTenantIdAndType`), decoupled from tenant-level preference; mandatory types fan out the same as any other type, nothing new needed there.
6. **Track per-channel delivery outcome on the `notifications` row itself**, not just existence — new `email_status` column (reusing the existing `EmailStatus` enum: PENDING/SENT/FAILED/SKIPPED), mirroring the already-established `documents.email_status` pattern. This makes a `notifications` row simultaneously the "did it happen" ledger, the in-app inbox entry, and the email delivery audit trail — one row, three questions answered.
7. **Also reconsider whether `PRICE_CHANGE_ANNOUNCED`'s in-app notification should stay synchronous** (`pricingService.notifyPendingPriceChangesForTenant` creates it directly rather than via a queued effect, specifically to avoid a race with itself across repeated calls — see ADR-023). Once every type's creation is unconditional, the same synchronous-creation pattern could arguably apply everywhere idempotent re-checking matters, or a different general solution (e.g. a claim/lock step) might be worth designing instead of special-casing. Worth a decision here rather than leaving price-change as the only synchronous one by accident.

**Migration surface:** one schema migration (`notification_preferences` channel column + `notifications.email_status`), touches the remaining 6 notification-creation call sites, the 2 preference endpoints + their validators, and every email-sending effect handler that currently has no channel check at all.

**Effort:** Medium–High — no new tables beyond the schema changes above, but broad and easy to get subtly wrong (missing a call site during the "always create" migration silently reintroduces the old bug for that one type). Should land as its own PR with full test coverage per call site, not bundled with unrelated feature work.
