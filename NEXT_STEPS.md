...
**Still open:**
- Trivy image scanning is wired into both deploy workflows but deliberately informational only (`exit-code: '0'`). **Triaged 2026-09-14** — full breakdown in `docs/security-audit-2026-09-12.md`'s finding #5: `node:20-slim` (84 findings) and `caddy:2-alpine` (40 findings) are both confirmed to already be on their tag's current latest published build (not a stale pin) — the findings are either genuinely unpatched upstream, waiting on an image rebuild we don't control, or (the 20 npm-CLI-bundle findings in `node:20-slim`) build-time-only tooling never invoked at runtime. `redis:7-alpine` is clean. Nothing actionable today; re-triage next time either base image's digest is bumped, and reconsider flipping `exit-code` to `'1'` (paired with a `.trivyignore` for whatever's still accepted at that point) only once the count meaningfully drops.
