...
## 2. Async Worker for SRI Submission (RabbitMQ) — done

Both phases are shipped: Phase 1 (queuing `POST /:key/send`/`GET /:key/authorize` through RabbitMQ) and Phase 2 (migrating every remaining fire-and-forget side effect — notifications, webhook fan-out, subscription hooks, transactional emails — onto the same durable mechanism, generalized into one `pending_effects` outbox table used by 14 async operations including SRI send/authorize itself). Subscription activation/tier-change/renewal reconciliation deliberately does **not** use this mechanism — it's handled by a periodic scan (`subscriptionService.applyPendingInvoiceLinks()`) inside `POST /v1/admin/jobs/subscriptions` instead, since firing a RabbitMQ effect on every document authorization system-wide to cover a check relevant to a small fraction of tenants wasn't worth it. See CLAUDE.md's "Async worker: pending_effects outbox" entry and ADR-019/ADR-022 (including its addendum) for the full design.

**Still open, not blocking:** turning each per-tenant/per-subscription unit of work inside the admin cron batch jobs (`POST /v1/admin/jobs/notifications`, `/jobs/subscriptions`, `/jobs/quota`) into its own queued `pending_effects` message (for parallelism and partial-failure resilience) is a reasonable future follow-up — those jobs are already decoupled from user-facing requests and run fine on their own external-cron schedule as-is.
