...
async function generateForTenant(tenantId, issuer = null) {
  ...
  for (const { document_type } of lightweight) {
    ...
    const row = await tenantAgreementModel.create({
      ...
      contentHash: agreementService.computeHash(rendered),
    });
...
async function validateTermsVersion(termsVersion) {
  let current;
  try {
    current = await agreementService.getCurrent('TERMS');
  } catch (err) {
    if (err.code === ErrorCodes.AGREEMENT_NOT_FOUND) return;
    throw err;
  }
...
async function acceptAll(tenantId, { ip, userAgent } = {}) {
  return tenantAgreementModel.acceptAllPendingByTenant(tenantId, { ip, userAgent });
}
...
async function getStatus(tenantId) {
  ...
  for (const template of templates) {
    const latest = await tenantAgreementModel.findLatestByTenantAndType(
      tenantId,
    ...
    if (!isAccepted) {
      outdated.push({
        ...
        url: `/v1/tenants/agreements/${template.document_type}`,
...
async function listForTenant(tenantId) {
  return tenantAgreementModel.findAllByTenant(tenantId);
}
...
async function renderForTenant(tenantId, documentType) {
  ...
  const doc = await tenantAgreementModel.findLatestByTenantAndType(
    tenantId,
    documentType
  );
  if (!doc) {
