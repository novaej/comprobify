...
const mockIssuer = { id: '00000000-0000-0000-0000-000000000005', sandbox: false };
...
describe('DocumentEmailService', () => {
  ...
  describe('retryFailedEmails', () => {
    test('returns zero counts when there are no pending emails', async () => {
      ...
      expect(documentModel.findPendingEmails).toHaveBeenCalledWith('00000000-0000-0000-0000-000000000005', false);
    ...
    test('sends a pending email, updates status to SENT, and logs EMAIL_SENT', async () => {
      ...
      expect(documentModel.updateStatus).toHaveBeenCalledWith('00000000-0000-0000-0000-000000000001', 'AUTHORIZED', {
        email_status: 'SENT',
      ...
      expect(documentEventModel.create).toHaveBeenCalledWith(
        '00000000-0000-0000-0000-000000000001', 'EMAIL_SENT', null, null, { to: 'buyer@example.com', retried: true }, null, '00000000-0000-0000-0000-000000000005', false
      );
      expect(result).toEqual({ sent: 1, failed: 0 });
    });

    test('records a failure, updates status to FAILED, and logs EMAIL_FAILED without throwing', async () => {
      const doc = { id: '00000000-0000-0000-0000-000000000002', status: 'AUTHORIZED', buyer_email: 'buyer@example.com' };
      documentModel.findPendingEmails.mockResolvedValue([doc]);
      ...
      expect(documentModel.updateStatus).toHaveBeenCalledWith('00000000-0000-0000-0000-000000000002', 'AUTHORIZED', {
        email_status: 'FAILED',
        email_error: 'Mailgun down',
      }, '00000000-0000-0000-0000-000000000005', false);
      expect(documentEventModel.create).toHaveBeenCalledWith(
        '00000000-0000-0000-0000-000000000002', 'EMAIL_FAILED', null, null, { error: 'Mailgun down', retried: true }, null, '00000000-0000-0000-0000-000000000005', false
      );
      expect(result).toEqual({ sent: 0, failed: 1 });
    });

    test('processes a mix of documents and tallies sent/failed independently', async () => {
      const doc1 = { id: '00000000-0000-0000-0000-000000000001', status: 'AUTHORIZED', buyer_email: 'a@example.com' };
  ...
  describe('retrySingleEmail', () => {
    ...
    test('throws when the document is not AUTHORIZED', async () => {
      documentModel.findByAccessKey.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', status: 'SIGNED' });
    ...
    test('marks email SKIPPED and returns no_email when the document has no buyer_email', async () => {
      documentModel.findByAccessKey.mockResolvedValue({ id: '00000000-0000-0000-0000-000000000001', status: 'AUTHORIZED', buyer_email: null });
      documentModel.updateStatus.mockResolvedValue({});

      const result = await documentEmailService.retrySingleEmail(accessKey, {}, mockIssuer);

      expect(documentModel.updateStatus).toHaveBeenCalledWith('00000000-0000-0000-0000-000000000001', 'AUTHORIZED', { email_status: 'SKIPPED' }, '00000000-0000-0000-0000-000000000005', false);
    ...
    test('treats DELIVERED and COMPLAINED as already sent too', async () => {
      documentModel.findByAccessKey.mockResolvedValueOnce({
        id: '00000000-0000-0000-0000-000000000001', status: 'AUTHORIZED', buyer_email: 'buyer@example.com', email_status: 'DELIVERED',
      });
      ...
      documentModel.findByAccessKey.mockResolvedValueOnce({
        id: '00000000-0000-0000-0000-000000000001', status: 'AUTHORIZED', buyer_email: 'buyer@example.com', email_status: 'COMPLAINED',
      });
    ...
    test('on send failure, updates status to FAILED, logs EMAIL_FAILED, and re-throws', async () => {
      ...
      expect(documentEventModel.create).toHaveBeenCalledWith(
        '00000000-0000-0000-0000-000000000001', 'EMAIL_FAILED', null, null, { error: 'SMTP timeout', retried: true }, null, '00000000-0000-0000-0000-000000000005', false
      );
    });
  });
});
