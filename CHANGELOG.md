...
### Added
- **`GET / POST / DELETE /api/keys`** — tenant-facing API key management. Mint multiple named keys (e.g. `frontend-prod`, `erp`, `mobile-app`), list them, and revoke leaked ones. `POST /api/keys` accepts `label` and optional `environment` (`sandbox` | `production`); production keys require at least one promoted issuer. `DELETE /api/keys/:id` cannot revoke the key being used for the request.
