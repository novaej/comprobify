# Region confirmed: nyc3, matching the comprobify-terraform-state Spaces bucket. This is
# where the bucket that stores Terraform's *state* lives - unrelated to the `region` var
# below, which controls where the production droplet itself gets provisioned (nyc1, same
# as staging). Staging and production share one Spaces bucket with different key prefixes
# (see "Remote state" in docs/terraform-digitalocean-setup.md) rather than two buckets.
#
...
terraform {
  backend "s3" {
    ...
    key                         = "production/comprobify/terraform.tfstate"
