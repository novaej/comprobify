...
> **The second half of this addendum is itself superseded by [ADR-027](027-decouple-activation-from-invoice.md)** (2026-08-23). The periodic-scan replacement described below, `applyPendingInvoiceLinks()`, no longer exists — ADR-027 removed the reason it was ever needed: `linkInvoice()` stopped triggering any subscription state transition at all (it's pure bookkeeping now — recording which document settles which payment), so there is no "was this authorized invoice linked before or after authorization" race left to reconcile. The `tenant_id` addition just below is unaffected and still current.
