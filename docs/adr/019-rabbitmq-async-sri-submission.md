...
## Addendum (2026-08-01): reconciliation cadence tightened from hourly to every 5 minutes

The "Consumers must be idempotent" bullet above notes the reconciliation interval as "currently hourly — see `render.yaml`" — that was accurate under the original Render Cron Job setup, where each invocation had a per-run cost that discouraged running it any tighter. After the move off Render to a self-hosted droplet cron (`docs/terraform-digitalocean-setup.md`), that cost no longer exists, so `POST /v1/admin/jobs/queue-reconciliation` now runs every 5 minutes — matching `config.queueReconciliation.effectStaleMinutes`'s own default — same as the Notifications job, and the tightest of the four admin jobs. See `docs/deployment.md`'s "Scheduled jobs" section for the full reasoning. No design change here, only a scheduling one — reconciliation still only ensures a message exists, never calls SRI itself.
