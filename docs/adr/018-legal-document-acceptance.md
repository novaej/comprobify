...
## Addendum (2026-07-25): `termsVersion` removed from `POST /v1/register`

The original "Validation against the published bundle happens server-side" decision above had `POST /v1/register` both validate a submitted `termsVersion` and stamp `tenants.agreement_accepted_at`/`agreement_version` immediately at tenant creation. On review, this didn't match what the endpoint actually does: nothing is accepted at registration — the per-tenant `tenant_agreements` snapshot for TERMS/PRIVACY/DPA isn't even generated until the fire-and-forget `TENANT_AGREEMENT_GENERATE` effect runs afterward, and it's created `PENDING`, not `ACCEPTED`. `generateForTenant()` always snapshots whichever template version is currently published at generation time — it never reads the `termsVersion` the client submitted, so that value had no effect on what was actually recorded. Stamping the denormalized cache at registration compounded this: it claimed acceptance the instant a tenant registered, ahead of the real acceptance step (`POST /v1/tenants/agreements`), which is the only place a tenant explicitly agrees to anything and the only place gating `POST /v1/tenants/promote` cares about.

`POST /v1/register` no longer accepts or validates `termsVersion` at all. `validateTermsVersion()` is now called from exactly one place — `POST /v1/tenants/agreements` — where the check is meaningful: a client fetched a specific published version to display, and the server confirms that's still current before recording real, explicit acceptance. `tenants.agreement_accepted_at`/`agreement_version` are now stamped only by that same call.

## Addendum (2026-08-01): `acceptAllPendingByTenant` scoped to only the currently-published version

`generateForTenant()` creates a new `tenant_agreements` row every time it's called against a template version the tenant doesn't have a row for yet (`ON CONFLICT DO NOTHING` on `(tenant_id, document_type, template_version)`), but never touches an existing `PENDING` row from an older, since-superseded version of the same `document_type` — that older row is simply left behind, still `PENDING`, once a newer version is published and lazily generated. In normal operation this is rare (a document type getting republished more than once before a given tenant re-accepts), but it did happen — and when it did, `acceptAllPendingByTenant()`'s original query (`UPDATE tenant_agreements SET status = 'ACCEPTED', ... WHERE tenant_id = $1 AND status = 'PENDING'`) accepted **every** `PENDING` row for the tenant regardless of version, not just the current one. A tenant who only ever saw and agreed to the latest DPA text ended up with every stale, superseded DPA version also marked `ACCEPTED`, stamped with the same `accepted_at`/`ip`/`user_agent` as the version they actually read — fabricating consent evidence for content they never saw.

Fixed by joining against `agreements` in the `UPDATE`: only a `tenant_agreements` row whose `(document_type, template_version)` matches a row in `agreements` with `is_current = true` is eligible to be accepted. A stale `PENDING` row from a superseded version can never match `is_current = true` (activating a new version flips it off, per `agreement.model.js`'s `activate()`), so it's left exactly as it was — still `PENDING`, orphaned, and harmless, since `findLatestByTenantAndType()` (what every read path actually uses) always resolves to the newest row per type regardless. No migration was needed; this is a query-scoping fix only, in `tenant-agreement.model.js`.
