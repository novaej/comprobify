...
- `NEXT_STEPS.md`'s Generic Repeated-Attempt / Anomaly Detection item lists registration recovery as its first wire-up target, for detecting unusually frequent legitimate recoveries (which could indicate a compromised certificate being reused) — a separate, later concern from the enumeration fix itself.
...
## Addendum (2026-09-19): `alreadyLinked` makes the rotate-and-demote path conditional

The addendum above assumed every genuine certificate match represents an actual recovery — the key was lost, the link was lost, or both. Once reserved (comprobify-web-internal) API keys shipped (ADR-034's migration-102 addendum), that stopped being true for one of `recover()`'s two real callers: comprobify-web calling it purely to *resync* its own stale local copy of a tenant's reserved key, when the tenant's local `Tenant`/`Issuer`/`TenantApiKey` link was never actually broken. Rotating a perfectly working key and forcing re-verification in that case was a disruptive, unrequested side effect with no security benefit — the caller already knew the tenant was linked; there was nothing to recover.

`recover()` now takes an `alreadyLinked` parameter (default `false`). On a genuine match, if `alreadyLinked` is `true`, it returns `{ ok: true, matched: true, alreadyLinked: true }` immediately — no key rotation, no `PENDING_VERIFICATION` demotion, no verification email. The `SUSPENDED` check and `RECOVERY_SUCCESS` attempt-tracking still run unconditionally beforehand, so anti-enumeration and suspension disclosure are unaffected either way. The genuine recovery path (missing/broken local link, `alreadyLinked` omitted or `false`) is unchanged from the addendum above — full rotate-and-demote, exactly as designed there.

This doesn't weaken the extra-validation property this addendum introduced: `alreadyLinked: true` is not a way to skip proving P12 possession — the match check (fingerprint comparison against `issuer.cert_fingerprint`) still runs first and is unaffected. It only skips the *consequence* of a match when the caller has independent, already-established knowledge that nothing was actually lost. See CLAUDE.md's "Account recovery" for the full current design, including comprobify-web's two `recoverAccountAction` branches.
