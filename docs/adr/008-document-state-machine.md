...
## Addendum (2026-07-13): `PENDING_SEND` state added for async SRI submission

The graph shown above (`SIGNED → RECEIVED | RETURNED` directly) reflected synchronous send/authorize. [ADR-019](019-rabbitmq-async-sri-submission.md) made that pipeline fully asynchronous and inserted a `PENDING_SEND` state between them: `SIGNED → PENDING_SEND → RECEIVED | RETURNED`. The dual-layer enforcement pattern this ADR decided on — application constants plus a DB trigger — is exactly what caught the risk of updating only one side: migration 074 had to update both `document-state-machine.js`'s `TRANSITIONS` map and the `enforce_document_state_transition()` trigger function in the same migration, or the DB would have rejected a transition the application layer considered valid. This is additive, not a reversal of the original decision — see CLAUDE.md Common Mistake #39 for the concrete failure mode this addendum is documenting.
