...
The tenant registers one or more HTTPS callback URLs (`POST /v1/webhooks`). The API pushes a signed payload to each subscribed endpoint immediately after a notification is created or updated. This is the canonical pattern for server-to-server API integration (Stripe, GitHub, Twilio).

**Polling (fallback / catch-up)**  
The consumer backend polls `GET /v1/notifications?sinceId=<id>` on a schedule. The `sinceId` cursor lets it catch up efficiently after any downtime without re-processing already-seen notifications.
...
Retries are processed by `POST /v1/admin/jobs/notifications`.
...
`GET /v1/notifications?sinceId=<id>` returns only notifications with `id > sinceId`. The consumer should store the highest `id` seen from each poll and pass it on the next request. This efficiently skips already-processed notifications without re-fetching them.
...
The consumer calls `POST /v1/notifications/:id/read` only when **all users** with access to the notification have marked it read on their side. At that point the notification is globally read and excluded from `unreadCount` on future polls.

### 8. Issuer filter via `X-Issuer-Id`

The consumer system knows which issuers a given user can access. `GET /v1/notifications` accepts an optional `X-Issuer-Id` header:
...
- **`POST /v1/notifications/sync` is removed.** The consumer no longer triggers cert checks. The admin job endpoint replaces it.
