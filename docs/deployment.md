...
7. Add a `comprobify-cron-production` env var group (`API_BASE_URL` only — no `sync: false` there, see below) and three `branch: production` cron services to `render.yaml` (mirroring the existing staging ones, each with its own `ADMIN_SECRET` `sync: false` entry), then sync the Blueprint. As brand-new resources with no prior environment assignment, they will need one manual step afterward to move them into the Production environment of the Comprobify project in the dashboard, plus setting each service's `ADMIN_SECRET` value by hand — see "Render Cron Job setup" under Scheduled jobs below
...
**`ADMIN_SECRET` is not declared in `render.yaml` at all — set by hand, directly in the dashboard, on the group.** Render's `envVarGroups` do not support `sync: false`: a secret declared there via YAML is silently dropped (no error, no prompt, it just never appears in the group). `ADMIN_SECRET` was instead added directly to the `comprobify-cron-staging` group in the Render dashboard, outside Blueprint management entirely — only the non-secret `API_BASE_URL` is actually declared in the file. This is a live test of whether a group variable *not* mentioned in `render.yaml` survives a Blueprint sync (whole resources absent from the YAML are never deleted on sync, per Render's docs — this checks whether that same guarantee extends to individual keys within a group). If it survives: every current and future cron job referencing this group inherits `ADMIN_SECRET` with zero dashboard steps, full stop — the best-case outcome. If a sync wipes it: fall back to declaring `key: ADMIN_SECRET` + `sync: false` directly on each of the 3 services instead (safe, but needs the value typed in once per service, present and future — see `render.yaml`'s own comment for exactly what that looks like).
...
**Adding a new scheduled job later:** add a new `type: cron` entry to the `services` list in `render.yaml` (see the existing three for the shape), referencing `comprobify-cron-staging` via `envVars: [{ fromGroup: comprobify-cron-staging }]`. Push to `main` and Render creates it. If the group-level `ADMIN_SECRET` experiment above held up, this is genuinely zero-touch for both vars; if it didn't, add the same per-service `ADMIN_SECRET` (`sync: false`) block the fallback approach uses, and set its value once for the new service.
