...
| `INTERNAL_SERVICE_SECRET` | No | Shared secret with `comprobify-web`'s BFF, letting it override `req.ip` with the real visitor's IP on calls it proxies server-side (`src/middleware/trusted-forwarded-ip.js`) — otherwise those calls resolve to the frontend's own hosting-platform egress IP. Unset means the feature is inactive and `req.ip` resolution is unchanged. Must match the value configured on the `comprobify-web` side exactly. |
