...
| `DOCS_BASE_URL` | Yes | `https://docs.comprobify.com` on both staging and production — the docs site is live with a real `/errors/*` section matching this exact path. When set, every RFC 7807 error response's `type` field links to `{DOCS_BASE_URL}/errors/{slug}` (`src/middleware/error-handler.js`); when unset, `type` falls back to a non-resolving relative `/problems/{slug}` placeholder, not `about:blank` — `buildTypeUrl()` has no RFC-7807-default fallback path. |
...
| `INTERNAL_SERVICE_SECRET` | Yes | Required at startup as of #208/ADR-035 — the API refuses to boot without it. Gates `POST /v1/register`/`/recover`/`/resend-verification`/the consuming `POST /v1/verify-email` to `comprobify-web`'s own server-side BFF only (`403 INTERNAL_SERVICE_ONLY` otherwise); direct third-party registration is no longer possible. Also lets that same BFF override `req.ip` with the real visitor's IP on calls it proxies server-side (`src/middleware/trusted-forwarded-ip.js`). Must match the value configured on the `comprobify-web` side exactly. |
...
| `SRI_MOCK_MODE` | No | Set to `true` on a staging deployment to skip the real SRI SOAP call entirely and fake an immediate `RECIBIDA`/`AUTORIZADO` for every document — useful for generating demo tenants/documents without depending on SRI's test environment. Structurally inert when `APP_ENV=production`, even if left set. The real SOAP call happens inside the `worker` process, not `api` — both containers already load the same `.env` via `env_file`, so setting it once covers both. |
...
| `REDIS_URL` | No | Backs the rate limiters' shared store (`src/services/redis.service.js`) so every API instance enforces one counter per key instead of counting independently. Unset falls back to `express-rate-limit`'s own in-memory store, correct for a single API instance. Self-hosted via `deploy/docker-compose.yml`'s `redis` service — see `docs/terraform-digitalocean-setup.md`. Also backs the repeated-attempt detection mechanism below — a no-op without it. |
| `AGREEMENTS_ENABLED` | No | Set to `false` to run without legal documents (Terms, Privacy, DPA). Defaults to `true`, so an existing deployment never silently loses its acceptance gate. When `false` the public agreement endpoints behave as if nothing is published, no `tenant_agreements` rows are generated at registration, and `POST /v1/tenants/promote` is not gated on acceptance. Admin publishing keeps working, so content can be prepared before flipping it on. Nothing is deleted — re-enabling restores what was already published and accepted. |
...
- [x] `trust proxy` in `server.js` matches the actual number of reverse proxy hops in front of the app (currently `2`: Cloudflare, then Caddy on the droplet) — required for IP-based rate limiters (`adminLimiter`/`registerLimiter`/`recoverLimiter`/`resendVerificationLimiter`) to see the real client IP via `X-Forwarded-For` instead of pooling all traffic into one bucket. Re-verify this number if the proxy chain ever changes. — confirmed in code (`src/server.js:19`), same for every environment
