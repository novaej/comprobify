...
| `BETTERSTACK_INGESTING_HOST` | No | Only needed if the Betterstack source's setup page shows a specific regional ingesting host (e.g. `https://s123456.eu-central-1a.betterstackdata.com`) rather than `@logtail/node`'s shared default — without it, syncing 401s against a host that was never issued your token |
...
**Triggers:** a push to `main` that touches anything under `terraform/**`, or manual `workflow_dispatch`. `deploy-staging.yml` also triggers on push to `main` now (staging deploys directly off it, no intermediate branch or tag — see "Releasing" in `CLAUDE.md`), but that workflow has **no** `paths` filter — it rebuilds/redeploys the app image on every push to `main`, including an infra- or docs-only one. That's a deliberate simplicity tradeoff, not an oversight: some files under `docs/` (`docs/agreements/*.md`, `docs/email-templates/*.txt`) are actually read at runtime inside the deployed container (see CLAUDE.md's Common Mistake #32), so a blanket `paths-ignore: [docs/**]` on the app deploy would silently skip redeploying exactly the changes that need it. The cost of the current approach is just an occasional redundant image build/deploy on a pure infra or docs change — harmless, if a little wasteful. This workflow (`terraform.yml`) stays a separate, `terraform/**`-scoped trigger regardless, so an infra-only PR doesn't need app code review to apply, and `main` review — plus the `staging-infra` Environment approval on `plan`/`apply` above — is the real gate for infra changes, independent of the app's own deploy cadence.
...
`resize_disk` is explicitly set to `false` on the droplet resource, overriding the provider's own default of `true`. DigitalOcean disk can grow but never shrink, regardless of this flag — leaving the default `true` means any temporary size-up-to-test permanently commits you to more disk (and its cost) even after sizing back down; a downsize attempt after that fails outright with `422 This size is not available because it has a smaller disk`. With `resize_disk = false`, a `droplet_size` change only touches CPU/RAM, leaving disk untouched, so scaling up and back down for testing stays fully reversible. To deliberately grow disk on purpose, flip `resize_disk` to `true` for that one `apply` (picking a `droplet_size` with more disk, e.g. a `-<N>gb` suffixed variant with the same CPU/RAM), then flip it back to `false` immediately after — that locks in the new, bigger disk as the reversible floor going forward.

**Troubleshooting: SSH works once, then resets on a second connection (e.g. CI deploys fail with "connection reset by peer" mid-handshake):**

Symptom: a single SSH connection to the reserved IP succeeds, but a second connection opened right after — or concurrently — gets reset during the handshake, with zero trace in `journalctl -u ssh` on the droplet (meaning it's reset before ever reaching `sshd`). This breaks `deploy-staging.yml`'s "Copy compose files to droplet" step specifically: `appleboy/scp-action` (via `drone-scp`/`easyssh-proxy`) opens two independent SSH connections per run — one to detect the remote OS, one for the actual file transfer — and the second one is the one that gets reset.

Before suspecting the reserved IP itself, rule out the mundane causes (all checkable over SSH as `cpfydeploy9x`, which has no `sudo` by design — for anything needing root, `docker run --rm --pid=host --privileged -it alpine nsenter -t 1 -m -u -n -i sh` gets a root shell via the deploy user's docker-group membership):
- `fail2ban-client status sshd` — confirm nothing is actually banned.
- `sshd -T | grep -i maxstartups` — confirm it's not throttling concurrent unauthenticated connections (default `10:30:100` is generous).
- `free -h` / `uptime` / `dmesg | grep -iE 'oom|killed process'` — resource exhaustion.
- The Cloud Firewall's actual applied rule for tcp/22, checked from the droplet's own Networking tab (not just the general Firewalls page) — confirm `0.0.0.0/0`.
- Resize the droplet up a tier (see above) and retest — if the exact same reset reproduces on a bigger box, that rules out resource pressure/size entirely.

If all of the above are clean and the reset still reproduces consistently — including from a residential IP, not just CI (test with two near-simultaneous connections: `ssh ... exit & ssh ... exit & wait`) — the most likely explanation is the specific reserved IP address itself. DigitalOcean draws reserved IPs from a shared pool, and a freshly-assigned one can carry leftover reputation/blocklist status from whatever it was used for previously; there's no dashboard signal for this. The only practical fix is `terraform destroy` + `apply` to get an entirely new reserved IP (see "Recreate it" above), then re-run the two-connection test against the new IP before trusting it and updating `DROPLET_IP`.
