...
**Status:** investigation complete; the mechanical, no-design-decision fixes were applied the same day (#4, #7, #8, #10, #11 below — see each item for what changed). **#1, #2, #3, #5, #6, #9, and #12 are now resolved** (2026-09-14) — see each entry for how. **#17 reviewed 2026-09-14 — no action needed for now**, see its entry. #13–16 remain open, tracked here.
...
| **Low** | 9 | `echo "${{ secrets.GITHUB_TOKEN }}" | docker login ... --password-stdin` on the droplet places the (ephemeral, narrowly-scoped) token as a literal process argument for an instant. GitHub's log masking prevents it reaching the Actions log. **Fixed 2026-09-14** — `${{ secrets.GITHUB_TOKEN }}` is no longer inlined directly into the script text; both `deploy-production.yml`/`deploy-staging.yml` now pass it via `appleboy/ssh-action`'s `envs: GHCR_TOKEN` input (set from the step's own `env:` block), and the script references `$GHCR_TOKEN` instead. | CI/CD |
...
| **Low** | 12 | The ephemeral `GITHUB_TOKEN` used for `docker login` on the droplet persists (base64, not encrypted) in `~/.docker/config.json` under the deploy user's home directory — a second secret-at-rest location outside the documented "one flat `.env` file" model. **Fixed 2026-09-14** — both deploy workflows now run `docker logout ghcr.io` immediately after `docker compose pull`, so the credential no longer lingers in `~/.docker/config.json` between deploys. | Deployment |
...
| **Informational** | 17 | Whether a dedicated secrets manager (Vault/AWS Secrets Manager/Doppler, etc.) is warranted, given secrets today live only as GitHub Environment Secrets/Variables written into the droplet's `chmod 600` `.env` on every deploy. **Reviewed 2026-09-14 — not yet warranted**, see its entry below for the reasoning and revisit triggers. | Deployment |
...
**Update (2026-09-14):** DigitalOcean's own native cluster backups (continuous base backup + WAL archiving, restore-to-new-cluster) are now the **primary** recovery mechanism, not a secondary layer — confirmed live via the dashboard's Restore-from-backup flow, full detail in `docs/guides/database-backups.md`. SnapShooter is being decommissioned: its free tier only allows a single backup slot per database, meaning no real retention or point-in-time history, which isn't a substitute for what DO's own layer already provides natively. It'll be deleted once nothing still depends on it; the manual `.sql.gz` restore procedure above remains documented as the historical record for as long as a SnapShooter backup might still exist. The one accepted trade-off: DO's native backups are same-region as the live cluster, so unlike SnapShooter's cross-region copy, they don't protect against a full regional DigitalOcean outage — not addressed here, tracked as an accepted gap.
...
**Deliberately informational for now (`exit-code: '0'`), not blocking.** `redis:7-alpine` came back clean (0 HIGH/CRITICAL).

**Triage (2026-09-14), `node:20-slim` — 84 HIGH/CRITICAL findings:**
- ~40 Debian 12 (bookworm) OS package CVEs with no fix published at all yet (`bsdutils`/`libblkid1`/`libmount1`/`libsmartcols1`/`libuuid1`/`mount`/`util-linux*` — one CVE cluster repeated across the util-linux package family, plus `gzip`, `libacl1`, `libsystemd0`/`libudev1`, `libtinfo6`/`ncurses-*`, `perl-base` — 3 CRITICAL + 5 HIGH, `zlib1g` CVE-2023-45853). Nothing to do — no patched version exists upstream yet for any deployment on this base.
- ~8 Debian OS CVEs with a fix that exists in Debian's repos but hasn't been picked up by an official `node:20-slim` rebuild yet (`libgnutls30` ×2 CRITICAL + 3 HIGH, `libpcre2-8-0` ×2 HIGH, `libcap2`). Confirmed via `docker buildx imagetools inspect node:20-slim` that our pinned digest **is** the tag's current latest published build — this isn't a stale pin, the upstream image itself lags its own base distro's security patches by some days/weeks.
- 20 findings entirely inside `/usr/local/lib/node_modules/npm/` (`tar`, `minimatch`, `glob`, `cross-spawn`, `pacote`, `sigstore`, `brace-expansion`, `ip-address`) — npm's own bundled CLI dependencies, confirmed via `docker run ... find / -name package.json`. This is build-time tooling only (`RUN npm ci --omit=dev`); the image's `CMD` is `node app.js`/`node workers/worker.js`, which never invokes `npm` — zero runtime attack surface, pure scan noise from an official image that happens to ship its own package manager.

**Triage (2026-09-14), `caddy:2-alpine` — 40 HIGH/CRITICAL findings:**
- 23 Alpine 3.23 OS package CVEs, all with a fix already in Alpine's repos (`curl`/`libcurl` ×9, `libssl3`/`libcrypto3` ×2, `c-ares`) — same situation as `node`'s libgnutls30 above: our pinned digest is confirmed to be the tag's current latest build, upstream just hasn't rebuilt the image against the patched packages yet.
- 17 CVEs compiled directly into the `caddy` binary itself (Go stdlib, `golang.org/x/crypto`/`x/net`/`x/text`, `google.golang.org/grpc`) — only fixable by Caddy's own maintainers rebuilding against a newer Go toolchain; nothing actionable on our side.

**Net effect: nothing in this baseline is fixable by us today** — it's either genuinely unpatched upstream, waiting on an image rebuild we don't control, or (the npm-bundle findings) not real runtime exposure at all. Making the gate hard-fail today would have permanently red-X'd every deploy for findings nobody can act on. Re-run this same triage (`trivy image --severity HIGH,CRITICAL`) the next time either base image's pin is bumped — if the count meaningfully drops (upstream catches up), reconsider flipping `exit-code` to `'1'`, at that point paired with a `.trivyignore` for whatever residual findings are accepted going forward. Tracked in `NEXT_STEPS.md`.
...
- **#9** (`docker login` token on process list) — **Fixed 2026-09-14**, see its entry above.
...
- **#12** (GHCR token in `~/.docker/config.json` on droplet) — **Fixed 2026-09-14**, see its entry above.
...
- **#17** — dedicated secrets manager: **not yet warranted.** Current setup is already access-controlled and encrypted at rest (GitHub Environment Secrets/Variables, per-environment), written fresh into the droplet's `.env` on every deploy (`chmod 600`, never committed — see this doc's own git-history scan in the intro, and `docs/deployment.md`'s security checklist). Single droplet per environment, single operator, and zero real production tenants yet (production remains blocked on the legal entity — see `docs/production-readiness-checklist.md`). A dedicated secrets manager earns its keep once any of: multiple `api`/`worker` instances need consistent secret distribution (the same horizontal-scaling trigger CLAUDE.md documents for `REDIS_URL` becoming load-bearing), the team grows past one operator and secret *access* needs scoping per-person instead of all-or-nothing droplet/GitHub-Environment access, automated rotation becomes necessary instead of manual SSH-in-and-edit (`scripts/rotate-encryption-key.js`, see finding #3, is the one rotation path that already exists and works without one), or a compliance posture (PCI-adjacent now that Payphone card payments exist; SOC2) requires a per-secret access audit trail finer-grained than what GitHub Environments provide. Revisit when any of those becomes true, or before onboarding the first paying production tenant if compliance requires it by then.
