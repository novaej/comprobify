...
- [ ] Set up the `production-infra` GitHub Environment (`DO_TOKEN`/`CLOUDFLARE_TOKEN`) — required-reviewer rule added *before* the secrets, never after. **Reverted to unchecked 2026-09-12**: the security audit (`docs/security-audit-2026-09-12.md` #1) verified live via `gh api` that `production`, `production-infra`, `staging`, and `staging-infra` all currently have zero protection rules, even though `DO_TOKEN`/`CLOUDFLARE_TOKEN` are already live in `production-infra` — the secrets landed without the reviewer rule ever actually being applied (or it was applied and later dropped). `terraform apply -auto-approve` against production currently runs with no human approval gate. Needs the rule added via Settings → Environments (or `gh api`) before re-checking this item — and needs a decision on who the required reviewer(s) should be.
