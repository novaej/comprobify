...
- Un intento fallido no necesita reintento manual al principio — `POST /v1/admin/jobs/queue-reconciliation` vuelve a publicar automáticamente el comprobante para que el worker lo intente de nuevo, hasta 5 intentos en total (`PENDING_EFFECTS_MAX_ATTEMPTS`). Si se agotan los 5, la fila `pending_effects` subyacente queda `FAILED` y deja de reintentarse sola — un administrador puede recuperarla vía `POST /v1/admin/jobs/pending-effects/:id/retry` (o `.../retry-failed` para todos los efectos `FAILED` a la vez) una vez que se cree resuelta la causa del fallo.
