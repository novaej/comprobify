...
Desde el cambio a envío asíncrono al SRI, `POST /:key/send` y `GET /:key/authorize` nunca llaman al SRI dentro de la solicitud — la llamada real ocurre después, en segundo plano, sin ningún cliente HTTP esperando su respuesta. Por lo tanto, `SRI_SUBMISSION_FAILED` ya no puede aparecer como cuerpo de respuesta RFC 7807 para ningún cliente. Un fallo de red ahora aparece como una fila `ERROR` en la bitácora de eventos del comprobante (`GET /:accessKey/events`) en su lugar — revisa ahí, no una respuesta HTTP, cuando un comprobante parezca estancado. Esta página se mantiene solo como referencia histórica, no como una respuesta que debas esperar analizar.
:::
...
- Un intento fallido no necesita reintento manual al principio — el sistema lo vuelve a intentar automáticamente, hasta 5 intentos en total. Si se agotan los 5, el comprobante queda estancado y tú mismo puedes recuperarlo — ver [Retry Send/Authorize](../endpoints/retry-send.md).
