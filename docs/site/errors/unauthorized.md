...
**What to do:** Use a key whose environment matches the tenant. List your active keys with `GET /api/keys`. Sandbox and production keys are issued separately — sandbox keys are created at registration; production keys are issued automatically at promotion (`POST /api/tenants/promote`) or minted manually afterwards.
