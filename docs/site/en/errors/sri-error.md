...
Since the change to asynchronous SRI submission, `POST /:key/send` and `GET /:key/authorize` never call SRI in-request — the actual call happens later, in the background, with no HTTP client waiting on it. `SRI_SUBMISSION_FAILED` can therefore no longer appear as an RFC 7807 response body to any client. A network failure now surfaces as an `ERROR` row in the document's event trail (`GET /:accessKey/events`) instead — check there, not an HTTP response, when a document seems stuck. This page is kept for historical reference only, not as a response you should expect to parse.
:::
...
- A failed attempt doesn't need manual retry at first — the system automatically retries it, up to 5 attempts total. If all 5 are exhausted, the document is stuck and you can recover it yourself — see [Retry Send/Authorize](../endpoints/retry-send.md).
