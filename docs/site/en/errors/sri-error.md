...
- A failed attempt doesn't need manual retry at first — `POST /v1/admin/jobs/queue-reconciliation` automatically re-publishes the document for another attempt by the worker, up to 5 attempts total (`PENDING_EFFECTS_MAX_ATTEMPTS`). If all 5 are exhausted, the underlying `pending_effects` row is marked `FAILED` and stops retrying on its own — an admin can recover it via `POST /v1/admin/jobs/pending-effects/:id/retry` (or `.../retry-failed` for every currently-`FAILED` effect at once) once whatever caused the failure is believed to be resolved.
