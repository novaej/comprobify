...
Any payment this creates is settled the same way as the initial subscription: by card from the Comprobify web app, or by uploading proof of an SPI transfer. Nothing is charged automatically — there are no recurring charges and no stored card. Which of three behaviors you get depends on whether the tier changes, the billing interval changes, or both:

- **Same-interval upgrade** (target tier's price higher than your current one, `billingInterval` omitted or unchanged) takes effect immediately, gated on payment. The price difference is prorated by the fraction of your current billing period remaining — e.g. upgrading exactly halfway through a monthly cycle charges roughly half the price difference. The response includes a `payment` and `bankTransfer` instructions, just like the initial subscription; upload proof via [`PATCH /v1/payments/:id/proof`](submit-payment-proof.md) (the same endpoint, no new upload flow). Your provider reviews it the same way as the initial activation — **the moment they verify the payment**, your tier flips and you keep the rest of the current billing period at the new tier (it doesn't restart). There is no wait on the provider's invoice being SRI-authorized. If the prorated amount rounds to **$0** (almost no time left in the period), the upgrade applies immediately with no payment step at all — there'd be nothing to send proof of.
...
The subscription itself (`tier`/`billing_interval`) does **not** change yet in this response — it only flips once the payment is verified and `current_period_end` arrives.
